...
//         dispatch(useGetAllCompanies()); // or re-fetch via custom hook
//       }
//     } catch (error) {
//       showErrorToast(error.response?.data?.message || "Error deleting company.");
//     }
//   };
...
//                   {/* <Avatar className="rounded w-10 h-10 bg-white border"> */}
//                   <Avatar className="rounded-none w-9 h-8 overflow-hidden">
//                     <AvatarImage
//                       src={
//                         company.logo ||
...
//                 <TableCell className="py-3">{formatDate(company.createdAt?.split('T')[0])}</TableCell>
...
//                         className="flex items-center gap-2 p-2 rounded hover:bg-red-100 cursor-pointer transition"
//                       >
  ...
  const handleDeleteCompany = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // showSuccessToast(data.message || 'Company deleted successfully');
      } else {
